...
describe('splitMultiline', () => {
	...
	it('should split a string longer than 80 characters into multiple lines', () => {
		const input =
			"This plugin is incompatible with the enabled WooCommerce features '%1//$s' and '%2//$s', it shouldn't be activated."
		const result = splitMultiline(input)
...
"and '%2//$s', it shouldn't be activated."`
	...
	it('should not exceed 66 characters per line when specified', () => {
		const input =
			'This is a specific test string that needs to be broken down into segments of no more than sixty-six characters.'
		const length = 66
		const result = splitMultiline(input, length)
		result?.split('\n').forEach((line) => {
			expect(line.length).toBeLessThanOrEqual(length + 2) // 2 is the length of the quotes before and after the string
		})
	})
})
